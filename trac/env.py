...
class Environment(Component, ComponentManager):
    ...
    secure_cookies = BoolOption('trac', 'secure_cookies', False,
        ...
        プションは Trac への接続に HTTPS が使える場合だけ、使用するようにしてください。 (''since 0.11.2 以降'')""")
    ...
    project_url = Option('project', 'url', '',
        """プロジェクトのメインとなる ウェブサイトの URL を設定します。
        通常、 `base_url` が示す ウェブサイトを指定します。この設定は通知メールで使用されます。""")
    ...
    log_file = Option('logging', 'log_file', 'trac.log',
        ...
        相対パスは TracEnvironment の `log` ディレクトリ"からの相対パスとして
        解決します。""")
    ...
    log_format = Option('logging', 'log_format', None,
        ...
        (http://docs.python.org/library/logging.html 参照) に加えて、以下のキーを使用することができます:
         - $(path)s     現在の環境へのパス 
